...
public class KNN {
	...
	public static byte[][][] parseIDXimages(byte[] data) {
		if(extractInt(data[0],data[1],data[2],data[3])!=magicImage) {
			System.out.println(extractInt(data[0],data[1],data[2],data[3]));
			return null ;
		}
		else{
			...
			while(index<data.length){
				...
				for(int i=0 ;i<height;i++){
					for(int j=0 ; j<width;j++){
						...
						tensor[imageIndex][i][j]=(byte)((data[index+j]&0xff)-128);
...
/************** End Process  Quick sort ***************/
	...
	public static byte electLabel(int[] sortedIndices, byte[] labels, int k) {
		...
		int k2=k;
		int i=0;
		while(i<sortedIndices.length && k2>0){
		...
		//Getting out the accuracy from vote tab in case we wont attend the K value 
		if(k2>0){
			ArrayList<Integer>valuesSupK=new ArrayList<Integer>();
		...
		return(labels[whoIsFirt[maxIndex]]);
	...
	public static byte knnClassify(byte[][] image, byte[][][] trainImages, byte[] trainLabels, int k) {
		...
		for(int i=0;i<trainImages.length;i++){
			...
			distancesTab[i]=invertedSimilarity(image,trainImages[i]);
		}
